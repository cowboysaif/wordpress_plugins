...
class EPub {
	...
	function wp_image_add_from_post ($post_content) {
  ...
  $re11='(\\s+)';	# White Space 3
  $re12='(src)';	# Word 4
  $re13='(=)';	# Any Single Character 4
  $re14='(".*?")';	# Double Quote String 3
  
    if ($c=preg_match_all ("/".$re1.$re2.$re3.$re4.$re5.$re6.$re7.$re8.$re9.$re10.$re11.$re12.$re13.$re14."/is", $post_content, $matches))
  {
      $c1=$matches[1][0];
      $word1=$matches[2][0];
      $ws1=$matches[3][0];
      ...
      $word3=$matches[8][0];
      $c3=$matches[9][0];
      $string2=$matches[10][0];
      $ws3=$matches[11][0];
      $word4=$matches[12][0];
      $c4=$matches[13][0];
      $string3=$matches[14][0];
    
  }

  $url = str_replace('"', '' , $string3 );
  $basename = basename($url);
  if ( $basename != '' ) {
  copy($url , $basename);
  $post_content = str_replace($url , $basename , $post_content );
 	$image = $this->getImage($basename);
	$this->zip->addFile($image['image'] , $basename );
  return  $post_content;
  }
  else {
	return $post_content;  
