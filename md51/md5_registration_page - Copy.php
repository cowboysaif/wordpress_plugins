...
<?php
	...
	if ( isset($_POST['customer_email']) && filter_var($_POST['customer_email'], FILTER_VALIDATE_EMAIL) && isset($_POST['customer_serial']) && isset($_POST['customer_order_id']) ) {
		...
		$product_number = $_POST['customer_order_id'];
		$hash = md5($product_number.$salt.$serial );
		$_SESSION[$_POST['customer_order_id']]['serial'] = $_POST['customer_serial'];
		$_SESSION[$_POST['customer_order_id']]['md5'] = $hash;
		generate_table();
		form_page();
		// disable this for now
		//email_customer($hash);
	}
	if ( isset($_POST['email_and_save'])) {
	}
...
<image style="width: 181px;" src=" '.WP_PLUGIN_URL.'/md51/images/'.get_option('md5_logo_file').'"></img></div>';
		...
		$msg = str_replace('[MD5 Key]', strtoupper($hash) , $msg);
...
<p>Serial number : '.$_POST['customer_serial'].'</p>
<p>Product number : '.$_POST['customer_order_id'].'</p>
<p>Hash : '.strtoupper($hash).'</p>';
		$msg = preparehtmlmail($msg , get_option('md5_company_email'));
 ...
 */
function fused_get_all_products_ordered_by_user($user_id=false,$status, $return_value ){
 
 $orders=fused_get_all_user_orders($user_id,'completed');
 if(empty($orders))
   return false;
 ...
 if ( $return_value == 'product_id' ) {
 $products=$wpdb->get_col($wpdb->prepare($query_select_product_ids,'_product_id'));
 }
 else {
 $products=$wpdb->get_col($wpdb->prepare($query_select_order_items,'_order_item_id'));	
 }
 return $products;
}

function generate_table() {
	$product_ids = fused_get_all_products_ordered_by_user(get_current_user_id(), 'completed', 'product_id' );
	...
	echo '<table class="myTable">
			...
			foreach ($product_ids as $id) {
			 ...
			 echo '<tr>
				<td>'.$id.'</td><td>'.$_SESSION[$_product->sku]['serial'].'</td><td>'.$_SESSION[$_product->sku]['md5'].'</td>
			</tr>';
	...
	<p/>
	<div style="padding-top: 2px; font-size: 89%">SKU</div>
	<input name="customer_order_id" type="text" size="42" required></input>
	<br/><br/>
	...
	<input type="Submit" value="Save and Email"></input>
	</div>
	</form>
	';
	}
